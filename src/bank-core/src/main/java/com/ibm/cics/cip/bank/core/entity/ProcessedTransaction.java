 ...
 * layer, never here.</p>
 *
 * <h2>{@code Persistable} insert-only optimisation (F2-04)</h2>
 * <p>This entity uses an <em>assigned</em> composite key ({@link #id} is set by
 * the service layer before {@code save()}, never generated by the database). For
 * an assigned-id entity, Spring Data's {@code SimpleJpaRepository.save(...)}
 * cannot tell a new row from a detached one by inspecting the id, so it issues a
 * defensive {@code SELECT} (an {@code EntityManager.find}) before every
 * {@code INSERT} to decide between {@code persist} and {@code merge}. Because
 * {@code processed_transaction} is strictly <strong>insert-only</strong> &mdash;
 * every append is a brand-new row and the {@link #deleted} flag is only ever set
 * at insert time (no code path mutates an existing PROCTRAN row) &mdash; that
 * existence check is pure overhead on the global audit-write hot path.</p>
 *
 * <p>Implementing {@link Persistable} and returning {@code true} from
 * {@link #isNew()} tells Spring Data the row is always new, so {@code save(...)}
 * routes straight to {@code EntityManager.persist(...)} and the redundant
 * {@code SELECT} is eliminated for every audit append (payment, transfer,
 * create, delete, and the data seeder).</p>
 *
 * <p><strong>Invariant:</strong> {@link #isNew()} is hard-coded {@code true}
 * precisely because PROCTRAN is append-only (ADR-006). Should a future
 * requirement ever wire up the logical delete, it MUST be performed with a
 * {@code @Modifying} {@code UPDATE} query (flipping {@link #deleted} in place),
 * <em>not</em> by loading and re-{@code save()}-ing the entity &mdash; with
 * {@code isNew()} fixed at {@code true}, a {@code save()} of an already-persisted
 * row would attempt a duplicate {@code INSERT} and violate the primary key.</p>
 */
@Entity
@Table(name = "processed_transaction")
public class ProcessedTransaction implements Persistable<ProcessedTransactionId>
{
	 ...
	 * {@inheritDoc}
	 *
	 * <p>Always returns {@code true} because {@code processed_transaction} is an
	 * append-only audit log (ADR-006): every {@code save(...)} of this entity is a
	 * fresh INSERT. Returning {@code true} lets Spring Data route {@code save(...)}
	 * directly to {@code EntityManager.persist(...)}, skipping the defensive
	 * existence-check {@code SELECT} that an assigned-id entity would otherwise
	 * incur on every audit append (F2-04). See the class-level documentation for
	 * the accompanying logical-delete invariant.</p>
	 *
	 * @return {@code true} always &mdash; this entity is only ever inserted
	 */
	@Override
