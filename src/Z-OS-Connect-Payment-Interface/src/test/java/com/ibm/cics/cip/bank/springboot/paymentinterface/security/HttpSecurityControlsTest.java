...
/*
 * V6 Missing HTTP Security Controls - response-level enforcement verification for the Payment
 * Interface module (closes QA test-coverage findings #1 and #4).
 *
 ...
 *     an authenticated TELLER POST to a money-movement endpoint WITHOUT a CSRF token must be
 *     rejected with HTTP 403. This proves CsrfFilter is active (CookieCsrfTokenRepository is NOT
 *     disabled); a valid TELLER principal is used so the 403 is attributable to the missing
 *     token, not to authentication/authorization. Covered for BOTH money-movement routes
 *     (/submit and /paydbcr).
 *
 ...
 *
 * All tests are offline and deterministic: the header assertion targets the GET "/" payment
 * form (no downstream z/OS Connect call) and the CSRF-negative assertions short-circuit at the
 * security filter chain. The CBSA_ZOSCONN_* stubs mirror the other security tests so the
 * application context boots consistently.
 */
@SpringBootTest
...
class HttpSecurityControlsTest
{
	...
	// ---- Finding #1: CSRF enforcement (tokenless money-movement -> 403) ----

	@Test
	@WithMockUser(username = "teller", roles = "TELLER")
	void authenticatedTellerSubmitWithoutCsrfTokenIsForbidden()
			throws Exception
	{
		// NOTE: intentionally NO .with(csrf()). An authorized TELLER is still rejected with 403
		// because the CsrfFilter denies the state-changing money-movement request that carries
		// no token. This is the negative (enforcement) path the success-only RBAC tests omit.
	...
	@Test
	@WithMockUser(username = "teller", roles = "TELLER")
	void authenticatedTellerPaydbcrWithoutCsrfTokenIsForbidden()
			throws Exception
	{
		mvc.perform(post("/paydbcr"))
				.andExpect(status().isForbidden());
	}
