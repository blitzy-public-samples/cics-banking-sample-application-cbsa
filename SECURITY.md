...
CBSA's security remediation defines the **security model** below. Every application-layer control described in this section is **in force in the current build**, with a single narrower-scope IDOR refinement still tracked as *Planned*:

- **In force now:** the Liberty container-managed authentication constraint on `/banking/*`; the per-method `@RolesAllowed("zosConnectAccess")` authorization on the Liberty JAX-RS resources (so that create, update, and delete operations require an authorized role) and the server-side IDOR entitlement hook (`callerEntitledToCustomer`) on the identifier-keyed Liberty lookups, enforced at Bank-Teller-role granularity; externalized credentials and the connection scheme; the CORS allowlist properties; property-level error-response sanitization (`server.error.include-stacktrace=never` and `server.error.include-message=never`); the centralized `@ControllerAdvice` exception handler in each Spring Boot module; the Spring Boot deny-by-default `SecurityFilterChain` (HTTP Basic, CSRF cookie token, CORS allowlist, and CSP header), wired into each module through the main-class component scan of the `config` package; and the per-operation `@PreAuthorize("hasRole('TELLER')")` restrictions on the state-changing Spring Boot handlers, backed by an externalized, provisioned `ROLE_TELLER` authority source (a `UserDetailsService` that reads `cbsa.security.teller.username` and `cbsa.security.teller.password`, and registers no user - denying access by default - when no teller password is supplied).
- **Planned:** narrower-principal IDOR restriction on the Liberty JAX-RS resources — tightening the entitlement check (currently enforced at Bank-Teller-role granularity) so that a principal without broad teller access cannot read another principal's account or customer records. Deeper *platform* hardening — additional z/OS Connect server hardening, BMS map and JCL security review, and secrets-scanning pipeline enhancements — likewise remains deferred and separately tracked; see [Reversibility and Rollback](#reversibility-and-rollback).

When assessing a deployment's current posture, note that every control listed under **In force now** is enforced; the item under **Planned** — narrower-principal IDOR restriction (and the deferred platform hardening) — is not yet enforced.
...
**Liberty z/OS Connect RESTful API (`webui`).** The Liberty tier uses container-managed authentication: a `security-constraint`/`login-config` in `src/webui/WebContent/WEB-INF/web.xml` (BASIC authentication, realm `zosConnect`, role `zosConnectAccess`) requires an authenticated principal for the `/banking/*` API, bound to a Liberty user registry. Per-method `@RolesAllowed("zosConnectAccess")` authorization is enforced on the JAX-RS resource methods, so that create, update, and delete operations require an authorized role; a caller lacking that role receives `403 Forbidden`.
...
> **CSRF-check ordering note.** Spring Security evaluates the CSRF token *before* HTTP Basic authentication. A state-changing request that omits a valid CSRF token is therefore rejected before the caller is authenticated, so under HTTP Basic it surfaces as `401 Unauthorized` rather than `403` (an established, already-authenticated session sees `403`). The request is rejected either way — CSRF protection is effective in both cases.
...
Lookups keyed on a client-supplied identifier (for example an account or customer `GET` by id) enforce a server-side ownership/entitlement check (`callerEntitledToCustomer`, derived from the authenticated principal via the JAX-RS `SecurityContext`) on the Liberty JAX-RS resources. Because CBSA is modelled from the point of view of a **Bank Teller**, that role legitimately has broad access to customer and account records, so the check is currently enforced at Bank-Teller-role granularity. *Planned:* tightening this entitlement check for narrower principals, so that a principal without broad teller access cannot read another principal's data. These server-side checks build on the Liberty JAX-RS authorization above.
...
|---------------------|-----------------------------|
| **A01 Broken Access Control** | Deny-by-default authentication, CSRF protection, and role-based authorization are enforced across the Spring Boot tier, where the `@PreAuthorize("hasRole('TELLER')")` guards are backed by a provisioned `ROLE_TELLER` authority source; the Liberty JAX-RS `@RolesAllowed("zosConnectAccess")` checks are enforced, and the server-side IDOR entitlement check is enforced at Bank-Teller-role granularity, with narrower-principal restriction planned (see the Implementation status note above). |
| **A02 Cryptographic Failures / A09 Security Logging & Monitoring Failures** | Error responses are sanitized at the framework property level — no stack traces or exception messages — and a centralized `@ControllerAdvice` exception handler in each Spring Boot module returns a generic response for genuinely uncaught exceptions, keeping exception detail (and PII) out of client responses while confining full diagnostics to the trusted server-side log, and keeping benign client errors (and their stack traces) out of the error log. |
